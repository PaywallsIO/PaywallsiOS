// https://github.com/perron2/swift-sqlite/blob/master/Database.swift
        ...
        for value in values.values {
            switch value.value {
                ...
                case let val as Date:
                    stmt.bind(value.name, val)
                default:
                    fatalError("Could not bind value for \(value.name)")
            }
        }
        try stmt.execute()
        return affectedRows
...
class DatabaseStatement {
    ...
    func bind(_ name: String, _ value: Data?) {
        let index = sqlite3_bind_parameter_index(stmt, ":" + name)
        assert(index > 0, "Invalid parameter name \"\(name)\"")
        ...
        if let value = value {
            _ = value.withUnsafeBytes { buffer in
                if let pointer = buffer.baseAddress {
                    sqlite3_bind_blob(stmt, index, pointer, Int32(buffer.count), SQLITE_TRANSIENT)
                } else {
                    sqlite3_bind_null(stmt, index)
                }
            }
    ...
    func bind(_ params: [Param]) {
        for param in params {
            ...
            switch value {
                ...
                case let value as Data: bind(name, value)
                default:
...
class DatabaseRows {
    ...
    func data(_ column: Int) -> Data? {
        if sqlite3_column_type(stmt, Int32(column)) == SQLITE_NULL {
            return nil
        }

        if let blob = sqlite3_column_blob(stmt, Int32(column)) {
            let blobLength = sqlite3_column_bytes(stmt, Int32(column))
...
class ContentValues {
    ...
    func put(_ name: String, _ value: Data?) {
        if let value = value {
            values.append(ContentValue(name, value))
        } else {
            values.append(ContentValue(name, nil))
